...
It also remembers. A relay that actually carried a connection on your phone goes to the front of the list next time, ahead of anything the feed rates higher — the feed's score is a global average and says nothing about whether a machine answers from your network, which is the only thing a connect is really asking. Same reasoning as Prowl's pool. The memory only reorders the list, so it can never pull you into a country you didn't pick, and it's forgotten after a fortnight, because that's about as long as a volunteer machine stays put.
